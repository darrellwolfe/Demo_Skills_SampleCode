...
# Need to read an excel file here
...
for row in rows:
    ...
    Excel_PIN, Excel_PermitNum, Excel_FileDate
    ...
    #SQL Query
    logging.info(f"Excel_PIN: {Excel_PIN}")
    logging.info(f"Excel_PermitNum: {Excel_PermitNum}")
    logging.info(f"Excel_FileDate: {Excel_FileDate}")
    ...
    if is_image_found(memos_land_information_, confidence=0.75):
        ...
        if popup_region:
            ...
            if click_on_image(memos_land_information_, direction='center', offset=0, confidence=0.75, region=popup_region):
