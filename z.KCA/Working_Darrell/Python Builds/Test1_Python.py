...
def get_window_region(window_title):
    try:
        ...
        logging.info(f"Window region for '{window_title}': {left}, {top}, {right}, {bottom}")
...
def click_on_image(image_path, direction='center', offset=0, inset=0, confidence=0.75, region=None):
    # Capture the screenshot using the specified region
    if region:
        logging.info(f"Capturing region: {region}")
    ...
    logging.info(f"Max value for image match: {max_val}")
    ...
    if popup_region:
        click_on_image("path/to/your/image.png", direction='center', region=popup_region)
    else:
