...
class StatusWindow:
    ...
    def update_status(self, status):
        ...
        self.current_record.set(f"{self.records_in_current_batch}/{self.batch_size}")
...
def main_script():
    ...
    for j, (index, row) in enumerate(records_to_process.iterrows()):
        ...
        if status_window.records_in_current_batch >= batch_size or j == len(records_to_process) - 1:
            save_progress()
