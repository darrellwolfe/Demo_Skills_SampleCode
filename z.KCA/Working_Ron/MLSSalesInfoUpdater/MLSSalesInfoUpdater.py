...
def setup_logging():
    ...
    # Create the MLSSalesInfoUpdater directory if it doesn't exist
    log_dir = os.path.join(user_home, 'MLSSalesInfoUpdater')
    os.makedirs(log_dir, exist_ok=True)
    
    # Create a log file name with the current date and time
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(log_dir, f'SalesInfoUpdater_{current_time}.log')
    
    # Set up logging with a format that includes date and time
    logging.basicConfig(
        filename=log_file, 
        level=logging.INFO,
    ...
    # Also set up a console handler to see logs in real-time
