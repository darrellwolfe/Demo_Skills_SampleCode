...
class StatusWindow:
    def __init__(self, master, total_records, remaining_records, batch_size):
        ...
        ttk.Button(master, text="Kill Script", command=self.kill_script).grid(row=6, column=0, columnspan=2, pady=10)
        ...
        self.update_status(0, 0, 0, 0, "Initializing...")

    def update_status(self, current_batch, total_batches, current_record, total_records, status):
        self.current_batch.set(f"{current_batch}/{total_batches}")
        self.remaining_batches.set(str(total_batches - current_batch))
        self.current_record.set(f"{current_record}/{total_records}")
...
def main_script():
    ...
    root = tk.Tk()
    status_window = StatusWindow(root, total_records, remaining_records, batch_size)

    if batch_size == 0:
        total_batches = 1
        batch_size = remaining_records  # Set batch size to total remaining records
    else:
    ...
    for j, (index, row) in enumerate(records_to_process.iterrows()):
        ...
        if not focus_proval_window('ProVal'):
            ...
            status_window.update_status(current_batch, total_batches, records_in_current_batch, batch_size, record_status)
            continue
        ...
        try:
            ...
            pag.write(AIN)  # Use pag.write instead of individual key presses
            ...
            records_in_current_batch = (j) % batch_size + 1
            status_window.update_status(current_batch, total_batches, records_in_current_batch, batch_size, record_status)
            
            status_window.increment_processed()  # Increment the processed count for every record

            if record_status != "Skipped or exited by user" and not record_status.startswith("Error:"):
                status_window.decrement_remaining()
                logging.info(f"Remaining records after processing: {status_window.remaining_records}")

        if batch_size != remaining_records and (j + 1) % batch_size == 0:
            df.to_excel(output_path, index=False, engine='openpyxl')
            logging.info(f"Updated output file after processing batch {current_batch}")

        if halt_event.is_set() or status_window.kill_flag:
            logging.info("Script termination requested during processing. Stopping.")
            break
