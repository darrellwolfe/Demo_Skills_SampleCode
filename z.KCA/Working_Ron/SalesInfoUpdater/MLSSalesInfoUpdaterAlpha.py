...
class StatusWindow:
    def __init__(self, master, total_records, remaining_records, batch_size, total_batches):
        self.master = master
...
def main_script():
    ...
    root = tk.Tk()
    status_window = StatusWindow(root, total_records, remaining_records, batch_size, total_batches)
    ...
    for j, (index, row) in enumerate(records_to_process.iterrows()):
        ...
        if status_window.records_in_current_batch >= batch_size:
            df.to_excel(output_path, index=False, engine='openpyxl')
            logging.info(f"Updated output file after processing batch {status_window.current_batch_number}")
            
            user_continue = messagebox.askyesno("Batch Complete", "Batch processing complete. Continue to next batch?")
