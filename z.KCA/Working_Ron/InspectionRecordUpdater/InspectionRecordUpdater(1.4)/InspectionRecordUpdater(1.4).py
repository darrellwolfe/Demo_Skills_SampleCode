...
class ProValUpdater:
    ...
    def update_records_batch(self):
        ...
        try:
            ...
            messagebox.showinfo("Batch Update Complete", f"Processed {total_records} records.\nSuccessful updates: {successful_updates}\nFailed updates: {total_records - successful_updates}")
            logging.info(f"Batch update completed. Processed {total_records} records. Successful: {successful_updates}, Failed: {total_records - successful_updates}")
        except Exception as e:
...
class SelectOrCreateMemoWindow(tk.Toplevel):
    def __init__(self, master, ry_memos, *args, **kwargs):
        ...
        self.y = self.screen_height - self.window_height - 200
    ...
    def create_widgets(self, ry_memos):
        ...
        content_height = self.scrollable_frame.winfo_reqheight() + button_frame.winfo_reqheight() + 20
        self.window_height = min(content_height, self.screen_height - 300)  # Adjusted to account for moving up
