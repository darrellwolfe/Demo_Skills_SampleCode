...
IF @CurrentDate > DATEFROMPARTS(@CurrentYear, 6, 1) --01/01/20xx
...
DECLARE @TaxYearPrevious INT = CAST(@Year - 1 as INT);
...
-- ,TAG
 ...
 Join TagTaxAuthority AS tta
   ...
   And tta.EffStatus = 'A'
   And tta.BegEffYear = (select max(BegEffYear) from TagTaxAuthority ttasub where ttasub.id = tta.id and ttasub.BegEffYear <= @Year)
 Join TaxAuthority AS ta
   On ta.id = tta.TaxAuthorityId
   And ta.EffStatus = 'A'
   And ta.BegEffYear = (select max(BegEffYear) from TagTaxAuthority tasub where tasub.id = ta.id and tasub.BegEffYear <= @Year)
 Join Taf
   On taf.TaxAuthorityId = ta.id
   And taf.EffStatus = 'A'
   And taf.BegEffYear = (select max(BegEffYear) from Taf tafsub where tafsub.id = taf.id and tafsub.BegEffYear <= @Year)
 Join Fund AS f
   On f.id = taf.fundId
   And f.EffStatus = 'A'
   And f.BegEffYear = (select max(BegEffYear) from Fund fsub where fsub.id = f.id and fsub.BegEffYear <= @Year)
...
SELECT
  lrsn,
  STRING_AGG(memo_text, ' | ') AS MemoText
FROM memos
...
LEFT JOIN CTE_AccountCreation AS ac
  On ac.lrsn = pmd.lrsn

LEFT JOIN CTE_TAG_TA_TIF_Key AS ttt
  On ttt.tag = pmd.tag
  LEFT JOIN CTE_TIFYear AS tify
    On ttt.TIFId = tify.Id
...
LEFT JOIN CTE_AumNotes_URD AS notesurd 
  On notesurd.lrsn = pmd.lrsn

LEFT JOIN CTE_Memos_URD AS memourd
  On memourd.lrsn = pmd.lrsn
