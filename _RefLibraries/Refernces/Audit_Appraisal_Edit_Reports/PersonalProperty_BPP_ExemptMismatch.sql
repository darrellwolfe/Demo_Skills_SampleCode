...
----------------------Main query becomes sub query to audit
...
-- Step 3: Identify owners with both 'Pos' and 'Neg' results
DuplicateOwners AS (
    SELECT
        Owner
    FROM GroupedData
    WHERE CheckColumn IN ('Pos', 'Neg')
    GROUP BY Owner
    HAVING COUNT(DISTINCT CheckColumn) = 2
),
