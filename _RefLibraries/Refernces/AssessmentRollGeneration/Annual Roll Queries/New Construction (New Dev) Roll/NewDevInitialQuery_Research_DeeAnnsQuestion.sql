...
And CAST(i.ValEffDate AS DATE) > @ValEffDateCurrent
--And CAST(i.ValEffDate AS DATE) = @EffYear0101PreviousLike
...
CTE_ValuationCurrent AS (
SELECT
v.lrsn
,v.valuation_comment AS PostedValuationCommentCurrent
,v.update_user_id AS update_user_idCurrent
,v.last_update AS last_updateCurrent
From valuation AS v
Where v.status = 'A'
And v.eff_year LIKE @EffYear0101CurrentLike
),

CTE_ValuationLast AS (
SELECT
v.lrsn
,v.valuation_comment AS PostedValuationCommentLast
,v.update_user_id AS update_user_idLast
,v.last_update AS last_updateLast
From valuation AS v
Where v.status = 'A'
...
,vc.PostedValuationCommentCurrent
,vc.update_user_idCurrent
,vc.last_updateCurrent

,vl.PostedValuationCommentLast
,vl.update_user_idLast
,vl.last_updateLast
...
LEFT JOIN CTE_ValuationCurrent AS vc
  On vc.lrsn = pmd.lrsn

LEFT JOIN CTE_ValuationLast AS vl
  On vl.lrsn = pmd.lrsn
...
, sac.SDYear
, sac.SHYear
, sac.SpecialHeader_Id
, sac.SpecialDetail_Id
, sac.ValueTypeId
, sac.Basis
...
Left Join CTE_SpecialAssessement_Current_Res AS sac
  On sac.lrsn = fnd.lrsn
...
Where (fnd.ImpValDetail_ImpId IN ('D','M','C')
  ...
  And fnd.ImpValDetail_YearBuilt = 2023

  And fnd.TotalImpLastYear = 0

  And (fnd.TotalImp > 0
       And fnd.TotalImp IS NOT NULL)
