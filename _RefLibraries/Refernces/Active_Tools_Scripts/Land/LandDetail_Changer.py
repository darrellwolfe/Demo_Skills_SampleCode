...
def toggle_entry_state():
    # Existing toggle logic for other fields...
    # Land Type
    ...
    # New toggle logic for Influence fields...
    # Influence
    if skip_influence_var.get():
        entry_influence.config(state='disabled')
    else:
        entry_influence.config(state='normal')

    # Type
    if skip_type_var.get():
        combo_type.config(state='disabled')
    else:
        combo_type.config(state='normal')

    # Adjustment
    if skip_adjustment_var.get():
        entry_adjustment.config(state='disabled')
    else:
        entry_adjustment.config(state='normal')
...
def setup_gui():
    global entry_parcels, entry_pricing_method, combo_land_type, entry_soil_type, entry_legend_type, entry_influence, combo_type, entry_adjustment, skip_land_type_var, skip_pricing_method_var, skip_soil_type_var, skip_legend_type_var, skip_influence_var, skip_type_var, skip_adjustment_var
    ...
    # Influence Entry
    ttk.Label(root, text="Enter Influence:").grid(column=0, row=5, padx=10, pady=5)
    entry_influence = ttk.Entry(root, width=30)
    entry_influence.grid(column=1, row=5, padx=10, pady=5)

    # Skip Influence Checkbox
    skip_influence_var = tk.IntVar()  # 0 means unchecked, 1 means checked
    skip_influence_checkbox = tk.Checkbutton(root, text="Skip Influence", variable=skip_influence_var, command=toggle_entry_state)
    skip_influence_checkbox.grid(column=2, row=5, padx=10, pady=5)

    # Type Dropdown
    ttk.Label(root, text="Select Type:").grid(column=0, row=6, padx=10, pady=5)
    type_options = ['1-Percent', '2-$Value']
    combo_type = ttk.Combobox(root, values=type_options, width=30)
    combo_type.grid(column=1, row=6, padx=10, pady=5)
    combo_type.current(0)  # Set default to first option

    # Skip Type Checkbox
    skip_type_var = tk.IntVar()  # 0 means unchecked, 1 means checked
    skip_type_checkbox = tk.Checkbutton(root, text="Skip Type", variable=skip_type_var, command=toggle_entry_state)
    skip_type_checkbox.grid(column=2, row=6, padx=10, pady=5)

    # Adjustment Entry
    ttk.Label(root, text="Enter Adjustment:").grid(column=0, row=7, padx=10, pady=5)
    entry_adjustment = ttk.Entry(root, width=30)
    entry_adjustment.grid(column=1, row=7, padx=10, pady=5)

    # Skip Adjustment Checkbox
    skip_adjustment_var = tk.IntVar()  # 0 means unchecked, 1 means checked
    skip_adjustment_checkbox = tk.Checkbutton(root, text="Skip Adjustment", variable=skip_adjustment_var, command=toggle_entry_state)
    skip_adjustment_checkbox.grid(column=2, row=7, padx=10, pady=5)

    # Update the submit button grid placement due to added fields
...
def on_submit():
    global total_parcels, selected_pricing_method, selected_land_type, selected_soil_type, skip_land_type, skip_pricing_method, skip_soil_type, skip_legend_type, LEGENDTYPE, LANDTYPE, INFLUENCE, INFLUENCETYPE, INFLUENCEADJUSTMENT
    ...
    # Get Influence if provided
    INFLUENCE = entry_influence.get().strip()

    # Get Type from dropdown
    INFLUENCETYPE = combo_type.get().strip()

    # Get Adjustment if provided
    INFLUENCEADJUSTMENT = entry_adjustment.get().strip()    
...
while current_parcel <= total_parcels:
    ...
    if not skip_influence_var.get():
        print("Begin Influence.")
        #skip_type_var, skip_adjustment_var
        stop_script

        #Click ADD 
        press_key_multiple_times('enter', 1)
        print("Press - enter.")
        time.sleep(1)

        #Typewrite Influence 
        pyautogui.typewrite(str(INFLUENCE))
        pyautogui.press('tab')
        print("Press - INFLUENCE.")
        time.sleep(1)

        #Dropdown Type 
        pyautogui.typewrite(str(INFLUENCETYPE))
        pyautogui.press('tab')
        print("Press - INFLUENCETYPE.")
        time.sleep(1)

        #Typewrite Adjustment Amount or Percentage
        pyautogui.typewrite(str(INFLUENCEADJUSTMENT))
        print("Press - INFLUENCEADJUSTMENT.")
        time.sleep(1)

        #Tab away before saving
        press_key_multiple_times('tab', 5)
        print("Press - tab.")

    else: 
        print("SKIP Influence.")
        #Tab away before saving
        #press_key_multiple_times('tab', 5)
        print("Press - tab.")
        time.sleep(1)
