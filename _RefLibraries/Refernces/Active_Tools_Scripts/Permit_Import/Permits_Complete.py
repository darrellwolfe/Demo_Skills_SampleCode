...
mylog_filename = 'C:/Users/dwolfe/Documents/Kootenai_County_Assessor_CodeBase-1/Working_Darrell/Logs_Darrell/PermitsComplete.log'
...
class UserInputForm:
    ...
    def __init__(self, root):
        ...
        self.APPRAISERINITIALS = ""
        self.COMPLETEDDATE = ""
        self.SELECTED_PARCEL_SET = ""
        
        self.setup_widgets()
        self.populate_parcel_set_dropdown()

    def ensure_capslock_off(self):
        # Implement your caps lock check logic here
        pass

    def on_submit(self):
        self.ensure_capslock_off()

        self.APPRAISERINITIALS = self.entry_initials.get()
        self.COMPLETEDDATE = self.entry_completeddate.get()
        self.SELECTED_PARCEL_SET = self.parcel_set_dropdown.get()

        if not self.APPRAISERINITIALS or not self.COMPLETEDDATE or not self.SELECTED_PARCEL_SET:
    ...
    def setup_widgets(self):
        vcmd = (self.root.register(self.validate_initials), '%a', '%Z')

        ttk.Label(self.root, text="Enter (3) Initials for Appraiser:").grid(column=0, row=0, padx=10, pady=5)
        self.entry_initials = ttk.Entry(self.root, width=50, validate='key', validatecommand=vcmd)
        self.entry_initials.grid(column=1, row=0, padx=10, pady=5)
        self.entry_initials.insert(0, "")

        ttk.Label(self.root, text="Completed Date:").grid(column=0, row=1, padx=10, pady=5)
        self.entry_completeddate = ttk.Entry(self.root, width=50)
        self.entry_completeddate.grid(column=1, row=1, padx=10, pady=5)

        ttk.Label(self.root, text="Select Parcel Set:").grid(column=0, row=2, padx=10, pady=5)
        self.parcel_set_dropdown = ttk.Combobox(self.root, width=47)
        self.parcel_set_dropdown.grid(column=1, row=2, padx=10, pady=5)

        submit_button = ttk.Button(self.root, text="Submit", command=self.on_submit)
        submit_button.grid(column=0, row=3, columnspan=2, pady=20)
...
form = UserInputForm(root)
initials = form.APPRAISERINITIALS
completed_date = form.COMPLETEDDATE
selected_parcel_set = form.SELECTED_PARCEL_SET
...
def click_on_image(image_path, direction='center', offset=0, inset=0, confidence=0.75):
...
query = f"SELECT DISTINCT TRIM(pm.AIN) AS AIN,TRIM(ps.set_id) AS set_id FROM Parcel_set AS ps JOIN TSBv_Parcelmaster AS pm ON ps.lrsn = pm.lrsn WHERE ps.set_id = '{selected_parcel_set}'"
...
for row in rows:
    ...
    logging.info(f"DBACRE: {DBPARCELSET}")


    #User Input
    logging.info(f"Initials: {initials }")
    logging.info(f"Completed_date : {completed_date }")
    logging.info(f"Parcel Set: {selected_parcel_set }")
    ...
    if is_image_found(memos_land_information_, confidence=0.75):
        ...
        # Click on Land Memo
        if click_image_single(memos_land_information_, direction='center', offset=100, confidence=0.75):
