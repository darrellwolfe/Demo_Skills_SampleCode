...
"""
This alt version is to fix the plats that appraisers "worked" without following the process. 
I think we can do the land memos and inspection records and land base,
but skip the permit and land details? I'll try this.
"""
...
def setup_logging():
    log_folder = os.path.join(os.path.expanduser('~'), 'MappingPacketes')
    if not os.path.exists(log_folder):
        os.makedirs(log_folder)
    log_file = os.path.join(log_folder, 'MLS_MappingPacket_SEGs.log')
...
def on_submit():
    ...
    global AINLIST, AINFROM, AINTO, PDESC, PFILE, PNUMBER, TREVIEW, MappingPacketType, MemoTXT, ForYear, AIN_Exclude, InitialsTech, InitialsAppraiser
    ...
    InitialsTech = entry_initials_tech.get().strip().upper()
    InitialsAppraiser = entry_initials_appraiser.get().strip().upper()
    ...
    MemoTXT = f"{InitialsTech}-{the_month}/{str(the_year)[-2:]} {MappingPacketType} from {AINFROM_str} into {AINTO_str} for {ForYear}"
    logging.info(f"Generated MemoTXT: {MemoTXT}")

    PDESC = f"{MappingPacketType} for {ForYear}"

    if not AINFROM or not AINTO or not PFILE or not PNUMBER or not TREVIEW or not MappingPacketType or not InitialsTech or not PDESC:
        # Does not need to include AIN_Exclude
        messagebox.showerror("Input Error", "All input fields are required.")

        if stop_script:
            logging.info("Stop flag detected. Exiting loop gracefully.")
            sys.exit()
...
def setup_widgets(root):
    global entry_ainfrom, entry_ainto, entry_pfile, entry_pnumber, entry_treview, combobox_mappingpackettype, entry_initials_tech, entry_initials_appraiser, for_year_combobox, entry_ainexclude
    ...
    # Initials with validation - TECH
    vcmd = (root.register(validate_initials), '%d', '%P')
    ttk.Label(root, text="Enter TECH (3) Initials:").grid(column=0, row=1, padx=10, pady=5)
    entry_initials_tech = ttk.Entry(root, width=50, validate='key', validatecommand=vcmd)
    entry_initials_tech.grid(column=1, row=1, padx=10, pady=5)
    entry_initials_tech.insert(0, "DGW")

    # Initials with validation - APPRAISER
    vcmd = (root.register(validate_initials), '%d', '%P')
    ttk.Label(root, text="Enter APPRAISER (3) Initials:").grid(column=0, row=2, padx=10, pady=5)
    entry_initials_appraiser = ttk.Entry(root, width=50, validate='key', validatecommand=vcmd)
    entry_initials_appraiser.grid(column=1, row=2, padx=10, pady=5)
    entry_initials_appraiser.insert(0, "DGW")

    # Mapping Packet Types
    ttk.Label(root, text="Select Mapping Packet Type:").grid(column=0, row=3, padx=10, pady=5)
    ...
    combobox_mappingpackettype.grid(column=1, row=3, padx=10, pady=5)
    combobox_mappingpackettype.current(0)  # Set default selection to the first item

    # Top Date
    ttk.Label(root, text="Filing Date (Top Date):").grid(column=0, row=4, padx=10, pady=5)
    entry_pfile = ttk.Entry(root, width=50)
    entry_pfile.grid(column=1, row=4, padx=10, pady=5)

    # Bottom Date
    ttk.Label(root, text="Permit Number (Bottom Date):").grid(column=0, row=5, padx=10, pady=5)
    entry_pnumber = ttk.Entry(root, width=50)
    entry_pnumber.grid(column=1, row=5, padx=10, pady=5)

    # AINFROM input
    ttk.Label(root, text="List AINs FROM (separated by comma):").grid(column=0, row=6, padx=10, pady=5)
    entry_ainfrom = ttk.Entry(root, width=50)
    entry_ainfrom.grid(column=1, row=6, padx=10, pady=5)

    # AINTO input
    ttk.Label(root, text="List AINs TO (separated by comma):").grid(column=0, row=7, padx=10, pady=5)
    entry_ainto = ttk.Entry(root, width=50)
    entry_ainto.grid(column=1, row=7, padx=10, pady=5)
    ...
    # AIN_Exclude input
    ttk.Label(root, text="Exclude these AINs (separated by comma) [Only use if starting over from the middle]:").grid(column=0, row=9, padx=10, pady=5)
    entry_ainexclude = ttk.Entry(root, width=50)
    entry_ainexclude.grid(column=1, row=9, padx=10, pady=5)

    # Submit
    submit_button = ttk.Button(root, text="Submit", command=on_submit)
    submit_button.grid(column=0, row=10, columnspan=2, pady=20)
...
def press_legend_key(LEGENDTYPE):
    # Attempt to convert LEGENDTYPE to an integer; if it's invalid or None, log it and default to 1
    try:
        if LEGENDTYPE is None:
            raise ValueError("LEGENDTYPE is None")
        LEGENDTYPE = int(LEGENDTYPE)
    except ValueError:
        logging.info(f"Invalid LEGENDTYPE: {LEGENDTYPE}. It must be an integer. Defaulting to 1.")
        LEGENDTYPE = 1

    # Mapping of LEGENDTYPE to the number of DOWN key presses
    legend_key_mapping = {
        0: 5,
        1: 5,
        2: 6,
    ...
    # Default to 5 DOWN key presses if LEGENDTYPE is not found in the mapping
    presses = legend_key_mapping.get(LEGENDTYPE, 5)
...
def determine_group_code(pcc_code):
    """
    Determines the land group code for the given PCC code.
    Returns a tuple: (homesite_code, remaining_acres_code)
    """

    # Mapping of PCC codes to Land Group Codes
    group_codes = {
        # Residential
        520: ("20", "20"),  541: ("20H", "20"),
        515: ("15", "15"),  537: ("15H", "15"),
        512: ("12", "12"),  534: ("12H", "12"),
        525: ("25L", "25L"),
        526: ("26LH", "26L"),  # Removed 'H' for Remaining Acres
    ...
    # Return (Homesite Code, Remaining Acres Code)
    return group_codes.get(pcc_code, ("UNKNOWN", "UNKNOWN"))
...
active_parcels_only_UNchecked = r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_open_active_parcels_only_UNchecked.PNG'
...
#PopUp
popup_Error_NoDataReturned = r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_popup_Error_NoDataReturned.PNG'
popup_Error_AppealAlreadyExists = r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_popup_Error_AppealAlreadyExists.PNG'
...
popup_FuturePropertyWarning = r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_popup_FuturePropertyWarning.PNG'

popup__OK = [
    r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_popup__OK_NotSelected.PNG',
    r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_popup__OK_Selected.PNG'
]
...
""" NEW WAIT SCREEN TOOLS """


def wait_for_screen(image_paths, timeout=30, check_interval=1, confidence=0.75):
    start_time = time.time()
    while time.time() - start_time < timeout:
        # If a list of images is provided, check each one
        if isinstance(image_paths, list):
            for img_path in image_paths:
                if is_image_found(img_path, confidence=confidence):
                    logging.info(f"Screen loaded: {img_path}")
                    return True
        else:
            # Otherwise, it's a single image path
            if is_image_found(image_paths, confidence=confidence):
                logging.info(f"Screen loaded: {image_paths}")
                return True
        time.sleep(check_interval)
    logging.warning(f"Timeout waiting for screen: {image_paths}")
    return False


"""
if not wait_for_screen(active_parcels_only_checked, timeout=60, check_interval=1, confidence=0.95):
    logging.error("Active parcels screen did not load in time.")
    # Optionally handle the error, retry, or exit the script.
...
def wait_for_window(window_title, timeout=30, check_interval=1):
    start_time = time.time()
    while time.time() - start_time < timeout:
        windows = gw.getWindowsWithTitle(window_title)
        if windows:
            logging.info(f"Window '{window_title}' found.")
            return windows[0]
        time.sleep(check_interval)
    logging.warning(f"Timeout waiting for window '{window_title}'.")
    return None


"""
window = wait_for_window('ProVal', timeout=60, check_interval=1)
if window:
    set_focus('ProVal')
else:
    logging.error("The 'ProVal' window did not appear in time.")
    # Optionally, handle the timeout scenario here (e.g., retry or exit)
...
def ProcessAIN():
    ...
    window = wait_for_window('Parcel Selection', timeout=30, check_interval=1)
    if window:
        set_focus('Parcel Selection')
        ...
        # We do NOT want Active Only for Parent Parecels
        if is_image_found(active_parcels_only_UNchecked, confidence=0.98):
            logging.info("Checked successfully active_parcels_only_checked no further action.")
            if stop_script:
                logging.info("Stop flag detected. Exiting loop gracefully.")
                sys.exit()
        ...
        # Error popup_Error_NoDataReturned
        # Check if the image is found and decide based on that
        if is_image_found(popup_Error_NoDataReturned, confidence=0.75):
            logging.info("Image was found - executing related tasks.")
            # Perform tasks related to the image being found

            # Click OK
            if click_images_multiple(popup__OK, direction='center', offset=100, confidence=0.75):
                logging.info("Clicked successfully popup__OK.")
                pyautogui.press('enter')
            if stop_script:
                logging.info("Stop flag detected. Exiting loop gracefully.")
                sys.exit()
        ...
        logging.info("set_focus")
        #Maximize Window
        # Simulate the Windows + Up Arrow key combination to maximize the windowl
        pyautogui.hotkey('win', 'up')
...
def LandBase():
    ...
    if not wait_for_screen(land_base_tab_images, timeout=60, check_interval=1, confidence=0.95):
        logging.error("Active parcels screen did not load in time.")
        # Optionally handle the error, retry, or exit the script.
...
def LandDetails():
    ...
    logging.info("Land Details Start HERE.")
    ...
    homesite_code, remaining_acres_code = determine_group_code(DBPCC)
    logging.info(f"Homesite Group Code: {homesite_code}, Remaining Acres Group Code: {remaining_acres_code}")


    LANDTYPE = None  # Initialize it to avoid unbound error
    logging.info(f"Reset LANDTYPE as None, then determine at next step: {LANDTYPE}")
    ...
    if LANDTYPE in ("RES_RURAL", "RES_URBAN","RES_HOMESITE"):
        ...
        if DBSITE > 0:
            logging.info(f"Yes DBSITE: {DBSITE}")
            logging.info(f"Also total_acres_not_cat19: {total_acres_not_cat19}")
    ...
    elif LANDTYPE in ("COMMERCIAL", "COMMERCIAL_CITY_LIMITS", "COMMERCIAL_RURAL_SUBDIVISION", "COMMERCIAL_RURAL_TRACT", "COMMERCIAL_CONDO", "INDUSTRIAL_CITY_LIMITS", "INDUSTRIAL_RURAL_SUBDIVISION", "INDUSTRIAL_RURAL_TRACT"):
...
def SetAllocations():
    ...
    window = wait_for_window('Cost Allocations', timeout=60, check_interval=1)
    if window:
    ...
    if is_image_found(allocations_9homesite_, confidence=0.75):
        ...
        if click_image_single(allocations_9homesite_, direction='below', offset=5, confidence=0.75):
            ...
            num_presses = pressess_allocations(homesite_code)
            logging.info(f"Using Homesite Code: {homesite_code} with {num_presses} presses.")
    ...
    if is_image_found(allocations_91RemAcres, confidence=0.75):
        ...
        if click_image_single(allocations_91RemAcres, direction='below', offset=5, confidence=0.75):
            ...
            num_presses = pressess_allocations(remaining_acres_code)
            logging.info(f"Using Remaining Acres Code: {remaining_acres_code} with {num_presses} presses.")
...
def InspectionRecords():
    ...
    # Send Initials
    pyautogui.typewrite(str(InitialsAppraiser))
    logging.info(f"User Initials {InitialsAppraiser}")
    ...
    """ ADD PERMITS - APPRAISERS """
def AddPermitsAppraiser():
    """ ADD PERMITS - APPRAISERS """
    ...
    if not wait_for_screen(permits_add_permit_button, timeout=60, check_interval=1, confidence=0.95):
        logging.error("Active parcels screen did not load in time.")
        # Optionally handle the error, retry, or exit the script.
        sys.exit()    
    ...
    window = wait_for_window('Add Permit', timeout=60, check_interval=1)
    if window:
        #set_focus('Add Permit')
        ...
        logging.info("I just sent the PNUMBER, I'm pressing TAB")
        ...
        logging.info("I just sent the PNUMBER, and Pressed Tab, I'm now pressing DOWN 4 times")

        #Different down to Timber 2 vs Mandatory 11 vs Seg/Combo 4
        press_key_multiple_times('down', 4)
        time.sleep(1)

        logging.info("I just pressed down 4 times, did I work?")
...
def NEW_Parcels():
    ...
    """ NEW Seg Parcels (NEW No Land Details Yet) """
    # Open AIN
    ...
    logging.info("Running def ProcessAIN")
    ProcessAIN()
    logging.info("End def ProcessAIN")


    # Land Base / Farm Acres

    if stop_script:
        logging.info("Stop flag detected. Exiting loop gracefully.")
        sys.exit()

    logging.info("Running def LandBase")
    LandBase()
    logging.info("End def LandBase")
    ...
    logging.info("Running def LandDetails")
    LandDetails()
    logging.info("End def LandDetails")


    # Allocations must be done only after Land Details

    if stop_script:
        logging.info("Stop flag detected. Exiting loop gracefully.")
        sys.exit()

    logging.info("Running def SetAllocations")
    SetAllocations()
    logging.info("End def SetAllocations")
    ...
    logging.info("Running def InspectionRecords")
    InspectionRecords()
    logging.info("End def InspectionRecords")
    ...
    logging.info("Running def AddPermitsAppraiser")
    AddPermitsAppraiser()
    logging.info("End def AddPermitsAppraiser")


    # Land Memos

    logging.info("Running def LandMemos")
    LandMemos()
    logging.info("End def LandMemos")
...
def ExistingParcels():
    ...
    """ Normal Seg Parcels (pre-existing) """
    # Open AIN
    if stop_script:
        logging.info("Stop flag detected. Exiting loop gracefully.")
        sys.exit()
...
""" THIS IS START OF THE SEG STUFF """
...
# Build the base query with parameter markers for AINLIST
query = f"""
Select Distinct 
...
# Append condition for AIN_Exclude if present
if AIN_Exclude:
...
# Combine parameters
params = AINLIST + AIN_Exclude

# Execute the fully built query once
cursor.execute(query, params)


global DBLRSN, DBPIN, DBAIN, DBPCC, DBPCCDESCR, DBTotalMktAcreage, DBTotalUseAcreage, DBACRE, DBCat19AC, DBSITE, DBREMACRES

# Process each row individually
for row in cursor.fetchall():
    if row:
        DBLRSN, DBPIN, DBAIN, DBPCC, DBPCCDESCR, DBTotalMktAcreage, DBTotalUseAcreage, DBACRE, DBCat19AC, DBSITE, DBREMACRES = row
        print(DBAIN, DBACRE)  # Example usage of variables
    ...
    logging.info(f"DBPCCDESCR: {DBPCCDESCR}")
    logging.info(f"DBTotalMktAcreage: {DBTotalMktAcreage}")
    logging.info(f"DBTotalUseAcreage: {DBTotalUseAcreage}")
    logging.info(f"DBACRE: {DBACRE}")
    logging.info(f"DBCat19AC: {DBCat19AC}")
    logging.info(f"DBSITE: {DBSITE}")
    logging.info(f"DBREMACRES: {DBREMACRES}")
    ...
    logging.info(f"InitialsTech: {InitialsTech}")
    logging.info(f"InitialsAppraiser: {InitialsAppraiser}")
    ...
    if DBTotalMktAcreage is None:
        print("Tasks for when DBTotalMktAcreage is None; PROCESS NEW")
        logging.info("Tasks for when DBTotalMktAcreage is None; PROCESS NEW")
        ...
        logging.info("Running def NEW_Parcels")
        NEW_Parcels()
        logging.info("End def NEW_Parcels")    


    elif DBTotalMktAcreage > 0:
        print("Tasks for when DBTotalMktAcreage is greater than zero; PROCESS EXISTING")
        logging.info("Tasks for when DBTotalMktAcreage is greater than zero; PROCESS EXISTING")
        ...
        logging.info("Running def ExistingParcels")
        ExistingParcels()
        logging.info("End def ExistingParcels")


    elif DBTotalMktAcreage == 0:
        print("Tasks for when DBTotalMktAcreage is 0 not null; PROCESS NEW")
        logging.info("Tasks for when DBTotalMktAcreage is 0 not null; PROCESS NEW")
    ...
    if stop_script:
        ...
        print("Invalid value for DBTotalMktAcreage")
        logging.info("Invalid value for DBTotalMktAcreage")


        # Handle unexpected case

    # If you have any final tasks that run regardless, place them here
    print("Final tasks running...")
    logging.info("Final tasks running...")
