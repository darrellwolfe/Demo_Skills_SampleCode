...
def setup_logging():
    ...
    log_file = os.path.join(log_folder, 'MLS_MappingPacket_PLATs_FullPacket.log')
...
def on_submit():
    ...
    global AINFROM, PDESC, PFILE, PNUMBER, TREVIEW, MappingPacketType, InitialsTech, InitialsAppraiser, MemoTXT, ForYear, PLATCOMBO, LEGENDTYPE, AIN_Exclude, PARENT_AIN_LIST
    ...
    # Split the string on commas and remove extra whitespace from each account
    PARENT_AIN_LIST = [acct.strip() for acct in AINFROM_str.split(",") if acct.strip()]
    ...
    MemoTXT = f"{InitialsTech}-{the_month}/{str(the_year)[-2:]} {MappingPacketType} from {AINFROM_str} into {PLATCOMBO} for {ForYear}"

    logging.info(f"Generated MemoTXT: {MemoTXT}")

    # Check if any required field is empty
    if not PLATCOMBO or not LEGENDTYPE or not PFILE or not PNUMBER or not TREVIEW or not MappingPacketType or not InitialsTech or not InitialsAppraiser or not ForYear:
        messagebox.showerror("Input Error", "All input fields are required.")
        return

    if not PLATCOMBO or len(PLATCOMBO) != 5:
        messagebox.showerror("Input Error", "PLATCOMBO must be exactly 5 characters long.")
        return
...
def validate_platcombo(action, value_if_allowed):
    # action == '1' means an insertion operation
    # action == '0' means a deletion operation
    if action == '1':
        # Allow insertion only if it would not exceed 5 characters
        return len(value_if_allowed) <= 5
    return True


def setup_widgets(root):
    global entry_ainfrom, plat_date, entry_treview, combobox_mappingpackettype, entry_initials_tech, entry_initials_appraiser, for_year_combobox, entry_ainexclude, entry_platcombo, entry_legendtype
    #combobox_landtype
    ...
    ttk.Label(root, text="Enter TECH (3) Initials:").grid(column=0, row=2, padx=10, pady=5)
    entry_initials_tech = ttk.Entry(root, width=50, validate='key', validatecommand=vcmd)
    entry_initials_tech.grid(column=1, row=2, padx=10, pady=5)
    entry_initials_tech.insert(0, "DGW")

    # Initials with validation - APPRAISER
    vcmd = (root.register(validate_initials), '%d', '%P')
    ttk.Label(root, text="Enter APPRAISER (3) Initials:").grid(column=0, row=3, padx=10, pady=5)
    entry_initials_appraiser = ttk.Entry(root, width=50, validate='key', validatecommand=vcmd)
    entry_initials_appraiser.grid(column=1, row=3, padx=10, pady=5)
    entry_initials_appraiser.insert(0, "DGW")

    # Filing Date
    ttk.Label(root, text="Date on first page of Plat packet:").grid(column=0, row=4, padx=10, pady=5)
    plat_date = ttk.Entry(root, width=50)
    plat_date.grid(column=1, row=4, padx=10, pady=5)




    # AINFROM input
    ttk.Label(root, text="List AINs FROM_PARENTs (separated by comma):").grid(column=0, row=5, padx=10, pady=5)
    ...
    ttk.Label(root, text="Enter the first 5 digits of the PLAT (PLATCOMBO):").grid(column=0, row=6, padx=10, pady=5)
    vcmd_platcombo = (root.register(validate_platcombo), '%d', '%P')
    entry_platcombo = ttk.Entry(root, width=50, validate='key', validatecommand=vcmd_platcombo)
    entry_platcombo.grid(column=1, row=6, padx=10, pady=5)


    ttk.Label(root, text="Enter Legend Type (Just the legend number, e.g., 1 for Legend 1):").grid(column=0, row=7, padx=10, pady=5)
    entry_legendtype = ttk.Entry(root, width=50)
    entry_legendtype.grid(column=1, row=7, padx=10, pady=5)
...
def ProcessAIN_Parents(ParentAIN):
    ...
    # Now using ParentAIN instead of DBAIN
    pyautogui.typewrite(f"{ParentAIN}")
    logging.info(f"Sent ParentAIN {ParentAIN}.")
    time.sleep(1)
    ...
    pyautogui.press('enter')
    time.sleep(1)
    logging.info(f"Close Pop-Up, Open the {ParentAIN}")
    # Close AIN Search Window
    if stop_script:
        logging.info("Script stopping due to kill key press.")
    check_stop_script()
...
""" Process Parent AIN Land Memos First """


# Iterate over each account number and perform your steps
for ParentAIN in PARENT_AIN_LIST:
    # Replace this print statement with the actual steps you need to perform
    print("Processing account:", ParentAIN)
    logging.info("Processing account:", ParentAIN)
    

    print("Open Parent AIN(s):", ParentAIN)
    logging.info("Open Parent AIN(s):", ParentAIN)
    ProcessAIN_Parents(ParentAIN)

    print("Send Land Memos:", MemoTXT)
    logging.info("Send Land Memos:", MemoTXT)
    LandMemos()
...
print("Now processing PLAT SQL")
logging.info("Now processing PLAT SQL")
...
for idx, row in enumerate(rows):
    ...
    try:
        ...
        logging.info(f"MemoTXT: {total_acres_not_cat19}")
