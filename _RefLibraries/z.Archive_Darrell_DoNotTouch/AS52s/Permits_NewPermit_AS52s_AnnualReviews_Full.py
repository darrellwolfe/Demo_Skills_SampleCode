...
def on_submit():
    global entry_ains, entry_file_date, combobox_permit_type, entry_initials_appraiser, combobox_start_end, combobox_StatusCode
    global AINLIST, PFILE, PDESC, PTYPE, PNUMBER, INITIALS, TREVIEW, START_OR_END, CORRECTEDNOTICE  # Add START_OR_END to globals
    ...
    START_OR_END = combobox_start_end.get().strip()  # Get the selected "Start" or "End" value

    CORRECTEDNOTICE = combobox_StatusCode.get().strip()
...
def setup_gui():
    ...
    ttk.Label(root, text="Select Permit Type:").grid(row=1, column=0, padx=10, pady=5)
    permit_types = [
        "Assessment Review"
    ]
    combobox_permit_type = ttk.Combobox(root, values=permit_types, width=47)
    combobox_permit_type.grid(row=1, column=1, padx=10, pady=5)
    combobox_permit_type.current(0)


    # File Date Input
    ttk.Label(root, text="File Date is Date On Form:").grid(row=2, column=0, padx=10, pady=5)
    entry_file_date = DateEntry(root, width=47, background='darkblue', foreground='white', borderwidth=2)
    entry_file_date.grid(row=2, column=1, padx=10, pady=5)
    ...
    # Start or End Dropdown
    ttk.Label(root, text="Select Start or End:").grid(row=4, column=0, padx=10, pady=5)
    start_end_options = ["Start", "End"]  # Options for the user to select
    combobox_start_end = ttk.Combobox(root, values=start_end_options, width=47)
    combobox_start_end.grid(row=4, column=1, padx=10, pady=5)
    combobox_start_end.current(0)  # Optionally set a default value


    # Start or End Dropdown
    ttk.Label(root, text="If End, Select CorrectedNotice or NoChange:").grid(row=5, column=0, padx=10, pady=5)
    StatusCode_options = ["", "CorrectedNotice", "NoChange"]  # Options for the user to select
    combobox_StatusCode = ttk.Combobox(root, values=StatusCode_options, width=47)
    combobox_StatusCode.grid(row=5, column=1, padx=10, pady=5)
    combobox_StatusCode.current(0)  # Optionally set a default value
...
Proval_permits_FieldPerson = r'S:\Common\Comptroller Tech\Reports\Python\py_images\Proval_permits_FieldPerson.PNG'
...
PressOK = [
    r'S:\Common\Comptroller Tech\Reports\Python\py_images\ProVal_OK_Selected.PNG',
    r'S:\Common\Comptroller Tech\Reports\Python\py_images\ProVal_OK_NotSelected.PNG'
]
...
def ok_to_continue():
    ...
    # Display the instruction dialog
    messagebox.showinfo("Click on the permit or appeal you wish to modify, then click OK to continue.")
    root.destroy()  # Clean up the root window to ensure it doesn't stay open
...
def ClosePermit():
    ...
    #Message box asking user to click on the permit they wish to turn off.
    ok_to_continue()


    # Click in the field next to Field Person, send initials
    if click_image_single(Proval_permits_FieldPerson, direction='right', offset=5, inset=5, confidence=0.70):
        logging.info("Clicked successfully Proval_permits_FieldPerson.")
        time.sleep(2)
    ...
    # Tab to Work Completed Date
    pyautogui.press('tab')
    time.sleep(0.25)

    # Set checkmark next to Work Completed Date, leave as today
    pyautogui.press('space')
    time.sleep(0.25)
...
def appeals_close():
    try:
        ...
        set_focus("Appeals")
        time.sleep(1)
        logging.info("Appeals Window")

        # Tab to Status Code
        pyautogui.press('tab')
        time.sleep(1)

        # Set Status Code to the top first
        pyautogui.press(['down'] * 10)
        time.sleep(1)

        # Status Code Option
        # AS-52 - Corrected Notice
        if CORRECTEDNOTICE == 'CorrectedNotice':
            # Set Status Code to #7, press down six times
            pyautogui.press(['down'] * 6)
            time.sleep(1)

        # AS-52 - No Change
        else:
            # Set Status Code to #6, press down five times
            pyautogui.press(['down'] * 5)
            time.sleep(1)
    
        # Tab to Assigned Initials
        pyautogui.press(['tab'] * 5)
        time.sleep(1)

        # Re-Set Appraiser Initials
        ensure_capslock_off()
        # Send Appraiser Initials
        pyautogui.typewrite(f'{INITIALS}')
        time.sleep(1)
        ...
        # Tab to Determination Type
        pyautogui.press(['tab'] * 16)
        time.sleep(1)


        # Set Determination Type to the top first
        pyautogui.press(['down'] * 10)
        time.sleep(1)


        # Set Determination Type to "Appraisal Review Forms AS52"
        pyautogui.press(['down'] * 4)
        time.sleep(1)


        # Press OK
        if click_images_multiple(PressOK, direction='center', inset=10, confidence=0.90):
            logging.info("Clicked successfully permits_tab_images.")
            time.sleep(2)
...
for row in rows:
    ...
    """ THE LOGIC BELOW RUNS ONE COURSE IF NEW APPEAL AND A DIFFERENT COURSE IF CLOSING APPEAL  """


    """ NEW APPEAL START """
    if START_OR_END == 'Start':
        logging.info("Processing START for New Appeal")
        # Add any specific logic for the start condition here
        ...
        """ APPEAL COMPLETE END PROCESS """

        logging.info("Processing END for Closing Appeal")
        # Add any specific logic for the end condition here
