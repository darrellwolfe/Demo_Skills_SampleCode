...
# Initialize tkinter root (hide the window)
root = tk.Tk()
root.withdraw()

# Add the pause and ask question where needed
def pause_and_ask(question):
    response = messagebox.askyesno("User Input Required", question)
    return response
...
def process_parcel():
    ...
    try:
        with connect_to_database(db_connection_string) as conn:
            with conn.cursor() as cursor:
                rows = execute_query(cursor, query)
                logging.info(f"Executed SQL query successfully")

                logging.info(f"Query {query}")

                # Separate rows with DBPermCount == 1 and others
                single_permit_rows = [row for row in rows if row[7] == 1]  # Assuming DBPermCount is the 8th value in the row
                multiple_permit_rows = [row for row in rows if row[7] > 1]


                # Process rows with DBPermCount == 1 first
                for row in single_permit_rows:
                    DBRefNum, DBDescr, DBAIN, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount = row
                    # Process each row with DBPermCount == 1
                    if not process_single_parcel(DBAIN, DBRefNum, DBDescr, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount):
                        break  # Stop processing if the script is terminated

                # After processing DBPermCount == 1, process the remaining rows
                for row in multiple_permit_rows:
                    DBRefNum, DBDescr, DBAIN, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount = row
                    # Process each row with DBPermCount > 1
                    if not process_multiple_parcel(DBAIN, DBRefNum, DBDescr, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount):
                        break  # Stop processing if the script is terminated
...
# Define the functions to process single and multiple parcels
def process_single_parcel(DBAIN, DBRefNum, DBDescr, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount):
    # Place the logic to process parcels with a single permit count here.
    # Return False if the script is to be terminated.
    ...
    #SQL Query
    logging.info(f"DBAIN: {DBRefNum}")
    logging.info(f"DBACRE: {DBDescr}")

    #User Input
    logging.info(f"AINLIST: {DBAIN}")
    logging.info(f"AINFROM: {DBPIN}")
    logging.info(f"AINTO: {DBSitus}")
    logging.info(f"MappingPacketType: {DBCity}")
    logging.info(f"Initials: {DBLRSN}")
    logging.info(f"ForYear: {DBPermCount}")
    ...
    pyautogui.press('enter')
    time.sleep(2)
    logging.info(f"Close Pop-Up, Open the {DBAIN}")
    stop_script
...
def process_multiple_parcel(DBAIN, DBRefNum, DBDescr, DBPIN, DBSitus, DBCity, DBLRSN, DBPermCount):
    # Place the logic to process parcels with multiple permits here.
    # Return False if the script is to be terminated.
    ...
    if DBPermCount > 1:
        ...
        if not pause_and_ask("Do you want to proceed with adding the permit information?"):
            logging.info("User chose to stop the process.")
            sys.exit("Script terminated by user request.")
    check_stop_script()
