...
def on_submit():
    ...
    PNUMBER = f"AS54 {entry_file_date.get().strip().upper()}"
    PDESC = f"AS-54 {entry_file_date.get().strip().upper()} REVIEW"
...
def setup_gui():
    global root, entry_ains, entry_file_date, combobox_permit_type
    root = tk.Tk()
    root.title("New Permit Input Form")
    
    # AINs Input
    ttk.Label(root, text="Enter AINs:").grid(row=0, column=0, padx=10, pady=5)
...
for row in rows:
    ...
    if click_image_single(add_field_visit_image_path, direction='center', inset=10, confidence=0.70):
        ...
        if click_image_single(permits_workassigneddate, direction='right', offset=5, inset=5, confidence=0.70):
            ...
            # Permit Due Date
            pyautogui.typewrite(f"12/31/{for_year}")
            time.sleep(0.25)
