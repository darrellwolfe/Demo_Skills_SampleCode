...
def select_permit_gui(record_data):
    ...
    for i, (key, value) in enumerate(record_data.items()):
        if key == "Address" and len(value) > 50:  # Adjust the threshold as needed
    ...
    root.geometry(f'+{x}+{y}')  # Set the position of the window
...
def process_hayden_email(message, success=True, skip=False):
    ...
    try:
        ...
        if success:
            message.FlagStatus = 1  # Set green checkmark flag
...
def process_csv(csv_path):
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            # Skip processing if the record is already completed
            if status == 'Completed':
...
def main():
    ...
    root.attributes('-alpha', 0)  # Make it completely transparent
    root.attributes('-topmost', True)  # Keep it topmost
    root.overrideredirect(True)  # Remove window decorations

    mode = gui_prompt("Processing Mode", "Choose processing mode:", ["CSV Input", "Hayden Email"])

    if mode is None:  # User closed the window without selecting
    ...
    if mode == "CSV Input":
        ...
        if csv_path:
            root.destroy()  # Destroy the root window before processing
